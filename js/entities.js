...
class Player {
  ...
  update(dt, tiles, boxes, buttons, windForce, crystals = []) {
    ...
    if (landedThisFrame) {
      if (typeof Renderer !== 'undefined') {
        ...
        if (this.isSlamming) {
          ...
          // ── Selar Fendas Temporais no momento do impacto ─────────────────
          // Checamos ANTES de zerar isSlamming, pois logo abaixo ele é apagado
          for (const fissure of (crystals || [])) {
            if (fissure && !fissure.activated && typeof fissure.seal === 'function') {
              const dist = Math.hypot(
                (this.x + this.w / 2) - (fissure.x + fissure.w / 2),
                (this.y + this.h) - (fissure.y + fissure.h / 2)
              );
              if (dist < 180) {
                fissure.seal();
              }
    ...
    // Tenebre: Selar fendas temporais ao passar por cima (mesma lógica dos cristais da Luxar)
    if (this.type === 'heavy') {
      for (const fissure of (crystals || [])) {
        if (fissure && !fissure.activated && typeof fissure.seal === 'function') {
          if (Physics.overlaps(this, fissure)) {
            fissure.seal();
          }
        }
      }
...
// ─────────────────────────────────────────────
// FISSURE — Selável por Tenebre com Slam
// ─────────────────────────────────────────────
class Fissure {
  constructor(config) {
    this.id = config.id;
    ...
    this.w = config.w ?? 64;
    this.h = config.h ?? 64;
    this.activated = false; // "activated" quer dizer selada!

    // Iluminação Dinâmica
    this.lightConfig = {
      radius: 150,
      intensity: 0.6,
      color: '#A85CFF', // Aura roxa para combinar com Tenebre
      flicker: true
    };
  }
  ...
  // seal() é o ponto de entrada do slam — simplesmente chama activate()
  seal() {
    this.activate(null);
  }
}
...
class MovingPlatform {
  constructor(config) {
    ...
    this.lightBridge = config.lightBridge ?? false; // Ponte de luz (ativada por botão)
    this._t = 0;
    this._dir = 1;
    this.prevX = this.x;
    this.prevY = this.y;
  ...
  update(dt) {
    ...
    // Ponte de Luz: fica estática, apenas muda active. Sem movimento.
