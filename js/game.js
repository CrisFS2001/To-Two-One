...
const Game = (() => {
  ...
  let state = {
    ...
    fissures: [],      // Fendas temporais do Tenebre
    portal: null,
    enemies: [],
  ...
  function loadLevel(idx) {
    ...
    // Fendas Temporais (Tenebre)
    state.fissures = (data.fissures || []).map(f => {
      const fissure = new Fissure({
        id: f.id,
        x: f.col * SCALER * TILE,
        y: f.row * SCALER * TILE,
      });
      if (window.GlobalState && window.GlobalState.isCrystalActivated(f.id)) {
        fissure.activated = true;
      }
      return fissure;
    });
    ...
    state.platforms = (data.platforms || []).map(p => {
      const plat = new MovingPlatform({
        ...
        lightBridge: p.lightBridge ?? false,
      });
  ...
  function update(dt) {
    ...
    // Vincula pontes de portal à ativação do portal
    for (const plat of state.platforms) {
      if (plat.id && plat.id.startsWith('ponte_portal') && state.portal && state.portal.active) {
        plat.active = true;
      }
    }
    ...
    const allTiles = [
      ...
      ...state.platforms
        .filter(p => {
          ...
          // Pontes de luz: só têm colisão quando ativas
          if (p.lightBridge && !p.active) return false;
          return true;
        })
    ...
    state.heavy.update(dt, allTiles, state.boxes, state.buttons, wind, state.fissures);
    ...
    // Ativa o portal quando todos os cristais ou fendas forem coletados/selados
    if (!state.portal.active) {
      const hasCrystals = state.crystals.length > 0;
      const hasFissures = state.fissures && state.fissures.length > 0;
      
      if (!hasCrystals && !hasFissures) {
        state.portal.active = true;
      } else {
        const crystalsActivated = !hasCrystals || state.crystals.every(c => c.activated);
        const fissuresActivated = !hasFissures || state.fissures.every(f => f.activated);
        if (crystalsActivated && fissuresActivated) {
          state.portal.active = true;
          console.log('Portal aberto! Cristais e Fendas estabilizados.');
        }
      }
    }
  ...
  function render(dt) {
    ...
    // Fendas Temporais (Tenebre)
    if (state.fissures) {
      for (const f of state.fissures) Renderer.drawFissure(f, ox, oy, state.time, dt);
    }
