...
const Renderer = (() => {
  ...
  function drawFissure(fissure, ox, oy, time, dt) {
    const cx = fissure.x + fissure.w / 2 + ox;
    const cy = fissure.y + fissure.h / 2 + oy;

    if (fissure.scale === undefined) {
      fissure.scale = fissure.activated ? 0.0 : 1.0;
    }

    if (fissure.activated) {
      fissure.scale = Math.max(0, fissure.scale - dt * 4.0);
    } else {
      fissure.scale = 1.0;
    }
    ...
    // Fenda ativa (não selada) ou fechando: desenha uma fenda instável roxa trêmula
    const idlePulse = (0.7 + 0.3 * Math.sin(time * 8 + (fissure.x * 0.05))) * fissure.scale; // pulso rápido para indicar instabilidade
    glow('#A85CFF', 15 * idlePulse);
    
    // Desenha a elipse/losango da fenda
    const w = 12 * idlePulse;
    const h = 48 * (1 + 0.1 * Math.sin(time * 15)) * fissure.scale;
    
    // Efeito de glitch de traçado
    ctx.strokeStyle = `rgba(168, 92, 255, ${(0.6 + 0.4 * Math.random()) * fissure.scale})`;
    ctx.lineWidth = 2 + Math.random() * 2;
    ctx.beginPath();
    ctx.ellipse(cx, cy, w, h, 0, 0, Math.PI * 2);
    ctx.stroke();
    
    // Preenchimento com gradiente de energia
    const grad = ctx.createRadialGradient(cx, cy, 0, cx, cy, h);
    grad.addColorStop(0, `rgba(168, 92, 255, ${0.7 * fissure.scale})`);
    grad.addColorStop(0.5, `rgba(168, 92, 255, ${0.2 * fissure.scale})`);
    grad.addColorStop(1, 'rgba(168, 92, 255, 0)');
    ctx.fillStyle = grad;
    ctx.beginPath();
    ctx.ellipse(cx, cy, w, h, 0, 0, Math.PI * 2);
    ctx.fill();
    ...
    // Anel instável
    ctx.strokeStyle = `rgba(255, 255, 255, ${0.3 * fissure.scale})`;
    ctx.lineWidth = 1;
    ctx.setLineDash([2, 4]);
    ctx.beginPath();
    ctx.arc(cx, cy, h * 0.8 * (idlePulse / (fissure.scale || 1)), 0, Math.PI * 2);
    ctx.stroke();
    ctx.setLineDash([]);
  ...
  function drawPlatform(plat, ox, oy, act) {
    ...
    // ── Ponte de Luz (lightBridge) — ativada por caixa em botão ─────────────
    // Inativa: invisível. Ativa: barra luminosa âmbar pulsante.
    if (plat.lightBridge) {
      if (!plat.active) {
        ctx.restore();
        return;
      ...
      ctx.shadowColor = `rgba(255, 220, 60, ${0.8 * pulse})`;
      ctx.shadowBlur = 18;

      // Linha principal da ponte
      const grad = ctx.createLinearGradient(bx, by, bx, by + bh);
      grad.addColorStop(0, `rgba(255, 240, 120, ${pulse})`);
      grad.addColorStop(0.5, `rgba(255, 210, 40, ${0.85 * pulse})`);
      grad.addColorStop(1, `rgba(255, 190, 20, ${0.6 * pulse})`);
      ctx.fillStyle = grad;
      ...
      ctx.shadowBlur = 6;
      for (let i = 0; i < 6; i++) {
        const px = bx + ((i * 113 + time * 40 * (1 + i % 2)) % bw);
        const py = by - 4 - (i % 3) * 4 - Math.sin(time * 3 + i * 1.2) * 4;
        const alpha = pulse * (0.4 + 0.6 * Math.abs(Math.sin(time * 2.5 + i)));
        ctx.globalAlpha = alpha;
        ctx.fillStyle = i % 2 === 0 ? '#ffe87c' : '#ffd040';
        ctx.beginPath();
  ...
  return {
    ...
    drawPortal, drawTether, drawPlayer, drawCrystal, drawFissure, drawLumaBridge,
